...
| 7 | `window.__TALQO__ = { token }` | Shorter, branded, object shape stays extensible without breaking changes |
...
```
1. Read widgetToken from window.__TALQO__.token
...
**`window.__TALQO__` type augmentation** (replaces `window.__AI_WIDGET_CONFIG__`):
...
- New embed code:
  ```html
  <script>
    window.__TALQO__ = { "token": "YOUR_TOKEN" };
  </script>
...
Widget init:
  Page loads → read window.__TALQO__.token → GET /widget/config
...
| Scenario | Behavior |
|----------|----------|
| `window.__TALQO__` missing or no token | `console.error`, widget does not mount |
