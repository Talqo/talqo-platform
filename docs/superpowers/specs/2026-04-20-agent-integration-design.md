...
**Lifecycle:**
1. **On mount:** generate `browserSessionId` UUID if not in `localStorage` (key: `pagepal:widget:session`). Call `POST /widget/sessions` with `{ browserSessionId }` → store returned `sessionId` in component state.
