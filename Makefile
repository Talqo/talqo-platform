...
# ── Helm ───────────────────────────────────────────
.PHONY: ns-create
ns-create: ## Create namespace from k8s/namespace-ENV.yaml if missing
	@out=$$(NAMESPACE="$(NAMESPACE)" RANCHER_PROJECT_ID="$(RANCHER_PROJECT_ID)" \
		envsubst < k8s/namespace-$(ENV).yaml | kubectl create -f - 2>&1) || { \
		case "$$out" in \
			*AlreadyExists*) printf '%s\n' "$$out" ;; \
			*) printf '%s\n' "$$out"; exit 1 ;; \
		esac; \
	}; \
	[ -z "$$out" ] || printf '%s\n' "$$out"
...
# ── Ops ───────────────────────────────────────────
.PHONY: _require-email
_require-email:
	@[ -n "$(EMAIL)" ] || { \
		echo "ERROR: EMAIL is required.  Usage: make create-admin EMAIL=admin@example.com PASSWORD=secret"; \
		exit 1; \
	}

.PHONY: _require-password
_require-password:
	@[ -n "$(PASSWORD)" ] || { \
		echo "ERROR: PASSWORD is required.  Usage: make create-admin EMAIL=admin@example.com PASSWORD=secret"; \
		exit 1; \
	}

.PHONY: create-admin
create-admin: _require-email _require-password ## Create an admin user in the deployed database (EMAIL=... PASSWORD=... required)
	@printf '%s\n' "$(PASSWORD)" | kubectl exec -i -n $(NAMESPACE) deploy/talqo-api -- \
		bun /app/apps/api/src/scripts/create-admin.ts "$(EMAIL)"
