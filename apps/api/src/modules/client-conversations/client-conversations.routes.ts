...
export const clientConversationRoutes = createRouter<{
	Variables: AppVariables
}>()

clientConversationRoutes.openapi(
	createRoute({
		method: "get",
		path: "/",
		tags: ["Client Account"],
	...
	async (c) => {
		...
		const result = await clientConversationService.listConversations(
			clientId,
			limit,
			offset,
		)
...
clientConversationRoutes.openapi(
	...
	async (c) => {
		...
		const result = await clientConversationService.getConversation(
			conversationId,
			clientId,
		)
		return c.json(result, 200)
