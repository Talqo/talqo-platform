...
- Monetary columns use `numeric` with `{ precision: 16, scale: 8, mode: "number" }` — `mode: "number"` makes Drizzle return JS `number` directly; DTOs use `z.number()` (or `z.number().nullable()`) for these fields. Note: `sum()` aggregates always return `string | null` regardless of column mode — wrap with `Number()` at the call site
...
- Response schemas always omit sensitive fields (`passwordHash`, `apiKeyEncrypted`, `widgetToken`) and add masked variants when needed
